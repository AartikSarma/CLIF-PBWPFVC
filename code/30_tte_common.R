# =============================================================================
# Script 30 (common): Longitudinal target trial emulation -- SHARED setup + compute
...
C_HIGH       <- as.numeric(Sys.getenv("PBWPFVC_TTE_CHIGH", "16"))  # permissive ceiling (~ usual care)
                        ...
                        # territory. Alternatives are swept in tte_sensitivities.R (strain-threshold
...
# TTE and the joint models (2x_biotrauma_*) share one grid and one pair of event
...
# it KEEPS the size correction validated by the race-invariance of specific elastance (xsec_age_correction_bracket E)
# and DROPS the age channel, which is both prognostically empty (xsec_age_correction_bracket D) AND the positivity-
...
# primary (or each other). Mirrors the cache-file suffix in 31_tte_engine.R.
...
# NOTE: the struct_excl CSV write is performed by 36_tte_diagnostics.R (this file is
...
arm_build <- function(ceiling, grace = GRACE, cap = DAYW_CAP, rule = "simple",
  ...
  # (linear) = the primary. tte_sensitivities.R passes a richer spec (ns(l_sf,3)+l_sf:disc_grp)
  # to address the residual stratum-varying l_sf imbalance the 37_tte_discordance_benefit TV-balance found. Additive:
  ...
  # VT/PFVC strain ceiling). 33_tte_ceiling passes a mechanical-power column ("mp_pfvc" / "mp_pbw") so
...
build_design <- function(c_low, c_high, grace = GRACE, cap = DAYW_CAP, rule = "simple",
  ...
  # different normalizations of the same quantity, as in 33_tte_ceiling's MP/PFVC vs MP/PBW).
  # sf_term likewise length 1 (shared) or 2 (per arm): 33_tte_ceiling gives each arm the SAME lagged-history
...
# =============================================================================
# 10e (design-build portion). PRIMARY design (the bootstrap itself lives in 35_tte_primary).
# =============================================================================
