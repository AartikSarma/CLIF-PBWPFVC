...
# script-03 outputs are missing, then runs code/29_run_biotrauma.sh. Sites run
# sequentially, so memory is one site's. Every knob of 29_run_biotrauma.sh
# (HORIZONS, FORMS, ITER, PAR, FRESH, ...) passes through the environment.
#
# Usage:
#   caffeinate -i nohup bash code/29_run_sites.sh \
...
[[ $# -ge 1 ]] || { echo "usage: bash code/29_run_sites.sh [--dry-run] NAME=TABLES_PATH[=FILE_TYPE] ..."; exit 1; }
...
for spec in "$@"; do
  ...
  if [[ $DRY == 1 ]]; then bash code/29_run_biotrauma.sh --dry-run | head -3; continue; fi
  bash code/29_run_biotrauma.sh > "$out/biotrauma.out" 2>&1
