# =============================================================================
# 11_ceiling_elastance: effect modification by MEASURED mis-sizing (specific elastance)
# =============================================================================
# Every modifier used so far -- PBW/PFVC discordance, PFVC itself -- is a deterministic
# function of age, sex, race and height (11.Z: R^2 ~ 0.99), so "who benefits" has been a
...
n_all <- nrow(base)
message(sprintf("11_ceiling_elastance [%s/%s]: %d of %d patients have a baseline plateau (E_spec); median E_spec %.1f cmH2O",
                FAM, DESIGN, nrow(emod), n_all, median(emod$spec_ers)))
if (nrow(emod) < 300) stop("11_ceiling_elastance: fewer than 300 patients with a baseline E_spec -- not estimable here.")
emod <- emod %>%
...
message("11_ceiling_elastance cluster bootstrap (", N_BOOT, " reps across ", nc_used, " core(s)) ...")
...
# --- console ---------------------------------------------------------------------------------
cat(sprintf("\n=== 11_ceiling_elastance [%s/%s]: %d patients ===\n", FAM, DESIGN, length(ids)))
