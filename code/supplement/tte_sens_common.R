# =============================================================================
# tte_sens_common: shared substrate for the split TTE sensitivity pieces
# =============================================================================
# Sourced by each tte_sens_*.R topic file (and by the tte_sensitivities.R run-all wrapper) so a new
# test can be run on its own WITHOUT rebuilding every other sweep. Defines the shared discordance-
# gradient estimator (identical to 37_tte_discordance_benefit) + a lazy VR-panel builder. Sourcing the engine (the
...
#
# Each piece writes the SAME output filenames as the old monolithic tte_sensitivities.R, so the
...
# standardized RD in tertile g (g="All" => overall), from a fitted pooled MSM -- identical to 37_tte_discordance_benefit
...
# Lazy VR-panel builder (the gas-subset dead-space confounder), used by tte_sens_deadspace and the
