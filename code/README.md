...
| `28_pre_period_placebo.R` | figure 4 | The pre-trend check: the lung-size divergence in the days before the first IMV record (panel `jm_pre_7d`, from `21`), beside the post-intubation rate; run by hand after reading the pre-period counts in `jm_panel_summary_7d_*` | `jm_pre_placebo_` |
