# =============================================================================
# 11_ceiling_common: shared substrate for the normalizer head-to-head TTE pieces (11_ceiling_*.R)
# =============================================================================
...
# deviation indicator, not in weight-model specification. Each design reports the 11.X reads:
...
#
# UNCERTAINTY. The design pieces (11_ceiling_headtohead.R / 11_ceiling_cap.R) carry a fixed-weight
# cluster bootstrap, which for two arms that share nearly all their clones conditions on one weight
# fit and is too narrow. 11_ceiling_diagnostics.R therefore runs a weight-REFIT bootstrap (both arms
...
#
# The MP family uses the 11.Y formula (0.098 x RR x VT[L] x PIP, daily median) so 11.Y and the MP
...
if (!exists(".ceil_loaded_key") || !identical(.ceil_loaded_key, .ceil_key)) {
  ...
  message(sprintf("11_ceiling [%s]: %d of %d patient-days carry the exposure (%.1f%% dropped); %d of %d patients",
                  FAM, x_drop$x_days, x_drop$panel_days, 100 * x_drop$frac_days_dropped,
                  x_drop$x_patients, x_drop$panel_patients))
  if (x_drop$x_patients < 100) stop("11_ceiling: fewer than 100 patients carry the exposure panel -- not estimable here.")
  ...
  if (thresholds$frac_days_disagree == 0)
    stop("11_ceiling: the two ceilings bind on exactly the same days -- no head-to-head contrast exists at this bite.")
  write_csv(bind_cols(thresholds, x_drop %>% select(-family)),
            file.path(final_dir, paste0(PFX, "ceiling_thresholds_", site_name, ".csv")))
  message(sprintf("11_ceiling [%s]: %s <= %.3g (~%.0f raw), %s <= %.3g (~%.0f raw); crossover discordance %.1f; %.1f%% of post-grace days disagree",
  ...
  if (cap_thresholds$frac_days_cap_only == 0)
    stop("11_ceiling: the PFVC cap never binds beyond the PBW ceiling -- no cap-on-top contrast at this anchor.")
  ...
  message(sprintf("11_ceiling [%s] cap: anchor discordance %.2f -> %s cap %.3g; cap binds beyond the PBW ceiling on %.1f%% of post-grace days (Concordant %.1f%% / Mid %.1f%% / Discordant %.1f%%)",
...
# =============================================================================
# 4. One full analysis per design: design build -> HTE (11.X read A) -> bootstrap -> tables -> figures
# =============================================================================
...
analyse_design <- function(design) {
  ...
  message("\n=== 11_ceiling [", FAM, "] design '", design, "': ", ARM["strain_limiting"], " vs ", ARM["permissive"], " ===")
  ...
  # --- HTE: pooled SOFA-adjusted IPC-weighted MSM, tertiles + continuous (11.X read A) ---------
  ...
  # (the weight-REFIT bootstrap in 11_ceiling_diagnostics.R supplies the primary interval)
  NCURVE <- length(EVAL)
  ...
  message("11_ceiling '", design, "' fixed-weight cluster bootstrap (", N_BOOT, " reps across ", n_cores_used, " core(s)) ...")
  boot_t0 <- Sys.time()
...
} else message("11_ceiling_common: already built this session for ", .ceil_key, " -- reusing.")
