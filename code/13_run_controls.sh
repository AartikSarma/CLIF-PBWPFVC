#!/bin/bash
# =============================================================================
# Script 13 (controls runner): every arm of the divergence-by-lung-size comparison
# =============================================================================
...
#
#   bash code/13_run_controls.sh anchors
...
#
#   SEV_MIN="platelets=2,bilirubin=1" bash code/13_run_controls.sh fits
...
#       no support above the floor. Then 13_control_comparison.R. Fits already on
...
run_arm () {      # name, then extra environment assignments for this arm
  ...
  run_step "${arm_name}_fit"    env "$@" Rscript code/13_biotrauma_fit.R
  run_step "${arm_name}_report" env "$@" Rscript code/13_biotrauma_report.R
}
...
case "$STAGE" in
  anchors)
    for COHORT in imv nosupport; do
      use_cohort $COHORT
      run_step "${COHORT}_panel"   Rscript code/13_biotrauma_panel.R
      run_step "${COHORT}_anchors" env PBWPFVC_JM_ANCHOR_ONLY=1 Rscript code/13_biotrauma_fit.R
    done
  ...
  fits)
    ...
    use_cohort imv
    run_step comparison Rscript code/13_control_comparison.R
    grep -A40 "divergence per day" "${LOG_DIR}/comparison.log" | grep -v "^Warning"
    ;;
  *) echo "usage: bash code/13_run_controls.sh anchors | SEV_MIN=... bash code/13_run_controls.sh fits"; exit 1 ;;
esac
