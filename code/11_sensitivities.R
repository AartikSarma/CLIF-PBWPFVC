...
# compute overall + per-tertile standardized RDs from a PRE-BUILT design (the shared estimator).
...
# =============================================================================
# 5. Ventilatory-ratio confounder sensitivity: dead-space test of the residual S/F imbalance
# =============================================================================
...
paco2_daily <- read_parquet(file.path(output_dir, "cohort_labs_clean.parquet")) %>%
  ...
  filter(vent_day >= 0, vent_day <= MAX_VENT_DAY) %>%
  group_by(hospitalization_id, vent_day) %>% summarise(paco2 = max(lab_value_numeric), .groups = "drop")  # daily worst
...
message("Wrote tte_ccw_disc_{ceiling,censoring}_sweep_ + sf_weightmodel_{balance,hte}_ + sf_weighttiming_ + vr_{hte,sfbalance}_",
        site_name, ".csv to ", final_dir)
